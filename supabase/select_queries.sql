...
WITH room_type_stats AS (
    SELECT 
        ...
        PERCENTILE_CONT(0.25) WITHIN GROUP (
            ORDER BY CASE WHEN TRIM(REPLACE(REPLACE(price, '$', ''), ',', '')) ~ '^-?[0-9]+(\.[0-9]+)?$' THEN TRIM(REPLACE(REPLACE(price, '$', ''), ',', ''))::NUMERIC(12, 2) ELSE NULL END
        ) AS q1,
...
SELECT 
    ...
    CASE 
        WHEN TRIM(REPLACE(REPLACE(l.price, '$', ''), ',', '')) ~ '^-?[0-9]+(\.[0-9]+)?$' 
        THEN TRIM(REPLACE(REPLACE(l.price, '$', ''), ',', ''))::NUMERIC(12, 2)
        ELSE NULL
    END AS price,
    CASE 
